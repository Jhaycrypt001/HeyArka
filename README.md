...
- The deployed canary runs the shield as it stood on 2026-09-15 and does **not** include the provenance gate — its service is deployed from the CLI, not from GitHub, so pushes do not reach it. Its agreement figures therefore measure one consistent shield version. The gate itself was measured at 0 false positives across 30 live feed items; redeploying the canary with it would start a new experiment, and should be reported as one rather than folded into the existing count.
